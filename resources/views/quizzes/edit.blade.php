<x-app-layout :title="'Edit Quiz - EduMind AI'">
    ...
    <div class="max-w-3xl mx-auto relative z-10">
        <x-glass-card class="p-8">
            ...
            <form action="{{ route('quizzes.update', $quiz->id) }}" method="POST" class="space-y-6 m-0 p-0">
                @csrf
                @method('PUT')
                <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="passing_score" name="passing_score" class="block w-full text-xs py-2.5 px-3" type="number" min="0" max="100" required value="{{ $quiz->passing_score }}" />
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" class="bg-slate-950/80 border-slate-800 text-slate-200 placeholder-slate-700 focus:border-purple-500 focus:ring-purple-500/30 rounded-xl shadow-sm transition-all duration-300 w-full text-xs p-3 focus:outline-none" required>{{ $quiz->description }}</textarea>
