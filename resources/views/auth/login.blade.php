<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div>
            ...
            <x-text-input id="email" class="block mt-1 w-full text-xs" type="email" name="email" :value="old('email')"
                required autofocus autocomplete="username" placeholder="name@domain.com" />
        ...
        <div class="flex items-center justify-between mt-4">
            <label for="remember_me" class="inline-flex items-center cursor-pointer">
                <input id="remember_me" type="checkbox"
                    class="rounded bg-slate-950 border-slate-800 text-purple-500 shadow-sm focus:ring-purple-500/20 focus:ring-offset-slate-900 focus:outline-none"
                    name="remember">
