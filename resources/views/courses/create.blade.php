        ...
        <x-glass-card class="p-8">
            ...
            <form action="{{ route('courses.store') }}" method="POST" class="space-y-6 m-0 p-0">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Category -->
                    <div>
                        <x-input-label for="category" :value="__('Syllabus Category')" />
                        <select id="category" name="category" class="bg-slate-950/80 border-slate-800 text-slate-350 focus:border-purple-500 focus:ring-purple-500/30 rounded-xl shadow-sm transition-all duration-300 w-full text-xs font-semibold py-2.5 px-3 focus:outline-none" required>
