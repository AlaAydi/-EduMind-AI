    ...
    <div class="admin-card p-5">
        <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-wrap gap-4 items-end">
            ...
            <div>
                ...
                <select id="role" name="role" class="form-input" style="width:150px">
            ...
            <div>
                ...
                <select id="status" name="status" class="form-input" style="width:160px">
                    <option value="">Tous</option>
                    ...
                    <tr class="border-b border-white/5 table-row">
                        ...
                        <td class="py-3 px-5">
                            <div class="flex items-center gap-3">
                                @if($user->avatar)
                                    <img src="{{ $user->avatar }}" class="w-9 h-9 rounded-full object-cover" alt="Avatar utilisateur">
                                @else
                        ...
                        <td class="py-3 px-5">
                            @if($user->is_approved)
                                <span class="px-2.5 py-1 rounded-full text-xs font-medium badge-approved">
                                    ✓ Approuvé
                                </span>
                            @else
                                <span class="px-2.5 py-1 rounded-full text-xs font-medium badge-pending">
                                    ⏳ En attente
                                </span>
                            @endif
                        </td>
                        ...
                        <td class="py-3 px-5">
                            <div class="flex items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.users.approve', $user) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="btn-success" style="padding:4px 10px;font-size:11px;">
                                        ✓ Approuver
                                    </button>
                                </form>
                                @endif

                                <a href="{{ route('admin.users.edit', $user) }}" class="btn-edit" style="padding:4px 10px;font-size:11px;">
                                    ✏️ Éditer
                                </a>
                                ...
                                <form method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-danger" style="padding:4px 10px;font-size:11px;">
                                        🗑 Suppr.
                                    </button>
                                </form>
