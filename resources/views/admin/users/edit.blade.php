...
<div class="max-w-2xl space-y-6">
    ...
    <div class="admin-card p-8">
        <div class="flex items-center gap-4 mb-8 pb-6 border-b border-white/5">
            @if($user->avatar)
                <img src="{{ $user->avatar }}" class="w-16 h-16 rounded-2xl object-cover" alt="">
            @else
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-5">
            ...
            <div class="flex items-center gap-4 p-4 rounded-xl" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07)">
                ...
                <div>
                    <label for="is_approved" class="text-white font-medium text-sm cursor-pointer">Compte approuvé</label>
                    <p class="text-gray-500 text-xs mt-0.5">L'utilisateur peut se connecter</p>
                </div>
            </div>

            <div>
                <label class="form-label">Nouveau mot de passe <span class="text-gray-600">(laisser vide pour garder l'actuel)</span></label>
